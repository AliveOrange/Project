...
class ChassisDeadReckoningWheel
{
public:
    ChassisDeadReckoningWheel(double distance_f2r, double distance_fr2l, double distance_rr2l,
                         double r_rrw, double r_rlw, double r_frw, double r_flw,
                         double front_wheel_weight, double rear_wheel_weight, double steer_transmission_ratio);
    ChassisDeadReckoningWheel() = default;
...
private:
    ...
    std::ofstream save_chassis_odom_wheel_;
};

ChassisDeadReckoningWheel::ChassisDeadReckoningWheel(double distance_f2r, double distance_fr2l, double distance_rr2l,
                                           double r_rrw, double r_rlw, double r_frw, double r_flw,
                                           double front_wheel_weight,
                                           double rear_wheel_weight,
                                           double steer_transmission_ratio) : front_wheel_weight_(front_wheel_weight),
...
bool ChassisDeadReckoningWheel::parse_chassis_odom(std::deque<odom::proto::Frame> &chassis_odom_queue)
{
...
void ChassisDeadReckoningWheel::init()
{
    char name[256] = {0};
    time_t curtime;
    time(&curtime);
    ...
    snprintf(name, 256, "%s_%d_%d_%d_%d_%d_%d.%s", "localization_log/odom_log/txt_log/chassis_odom_wheel", year, month, day, hour, min, sec, "txt");
    save_chassis_odom_wheel_.open(name, ios::app);
    save_chassis_odom_wheel_.setf(ios::fixed, ios::floatfield);
    save_chassis_odom_wheel_.precision(15);
    save_chassis_odom_wheel_ << "1 time, 2 x, 3 y, 4 z, 5 vx, 6 vy, 7 vz, 8 qx, 9 qy, 10 qz, 11 qw " << std::endl;
}
bool ChassisDeadReckoningWheel::publish_chassis_odom(std::string type)
{
...
void ChassisDeadReckoningWheel::linear_speed_and_angle_speed_compute_()
{
    if (vcu_vehicle_info.gear_status == 4 || vcu_vehicle_info.gear_status == 5) //前行档位，x轴向速度为正
    {
        ...
        // 当前时刻局部坐标系角速度计算，根据轮速计算yaw角速度方向与大小
        if (vcu_vehicle_info.rear_right_wheel_speed - vcu_vehicle_info.rear_left_wheel_speed != 0)
        {
            current_dr_transform.angle_speed.roll = 0;
            current_dr_transform.angle_speed.pitch = 0;
            double front_wheel_w = (vcu_vehicle_info.front_left_wheel_speed * vehicle_info_.r_flw * sin(theta) +
                                    vcu_vehicle_info.front_right_wheel_speed * vehicle_info_.r_frw * sin(theta)) /
                                   (2 * vehicle_info_.distance_f2r); //前轮平均yaw角速度
            double rear_wheel_w = (front_wheel_weight_ *
                                       (vcu_vehicle_info.front_right_wheel_speed * vehicle_info_.r_frw * sin(abs(theta)) -
                                        vcu_vehicle_info.front_left_wheel_speed * vehicle_info_.r_flw * sin(abs(theta))) /
                                       vehicle_info_.distance_fr2l +
                                   rear_wheel_weight_ *
                                       (vcu_vehicle_info.rear_right_wheel_speed * vehicle_info_.r_rrw -
                                        vcu_vehicle_info.rear_left_wheel_speed * vehicle_info_.r_rlw) /
                                       vehicle_info_.distance_rr2l);
            //后轮平均yaw角速度
            current_dr_transform.angle_speed.yaw = (front_wheel_weight_ * front_wheel_w + rear_wheel_weight_ * rear_wheel_w); //整车综合yaw角速度
        }
        else if (vcu_vehicle_info.steer_wheel_angle == 0) //直行
        {
            current_dr_transform.angle_speed.roll = 0;
            current_dr_transform.angle_speed.pitch = 0;
        ...
        if (vcu_vehicle_info.rear_right_wheel_speed - vcu_vehicle_info.rear_left_wheel_speed != 0)
        {
            ...
            double rear_wheel_w = (front_wheel_weight_ *
                                   ...
                                   rear_wheel_weight_ *
                                       (vcu_vehicle_info.rear_right_wheel_speed * vehicle_info_.r_rrw -
                                        vcu_vehicle_info.rear_left_wheel_speed * vehicle_info_.r_rlw) /
                                       vehicle_info_.distance_rr2l);                                                           //后轮平均yaw角速度
            current_dr_transform.angle_speed.yaw = -(front_wheel_weight_ * front_wheel_w + rear_wheel_weight_ * rear_wheel_w); //整车综合yaw角速度，倒车时整车姿态变化与前行时的方向相反
        }
        else if (vcu_vehicle_info.steer_wheel_angle == 0) //直行
        {
            current_dr_transform.angle_speed.roll = 0;
...
void ChassisDeadReckoningWheel::DeadReckoningWithChassis()
{
    std::lock_guard<std::mutex> lock(chassis_odom_mutex_);
    ...
    for (size_t i = 0; i < chassis_queue.size(); i++)
    {
        if (status_chassis_ == 0) //底盘里程计初始状态
        {
            ...
            current_chassis_odom.mutable_header()->set_frame_id("chassis_odom_wheel");
            ...
            if (test_status)
            {
                save_chassis_odom_wheel_ << chassis_queue.at(i).header().timestamp_sec() / (double)1000000<<","<<
                0<<"," <<
                0<<"," <<
                0<<"," <<
        ...
        else if (status_chassis_ == 1)
        {
            ...
            result_dr_transform.position_matrix = last_dr_transform.position_matrix + q.toRotationMatrix() * delta_dr_transform.position_matrix;
            ...
            odom::proto::Frame current_chassis_odom;
            current_chassis_odom.mutable_header()->set_odom_id(odom::proto::OdomId::ODOM_CHASSIS);
            current_chassis_odom.mutable_header()->set_sys_timestamp(vcu_vehicle_info.timestamp);
            current_chassis_odom.mutable_header()->set_rolling_counter(rolling_counter_);
            current_chassis_odom.mutable_header()->set_frame_id("chassis_odom_wheel");
            current_chassis_odom.mutable_header()->set_ins_utm_timestamp(vcu_vehicle_info.timestamp*1000000);
...
//角度增量转为旋转矩阵
void ChassisDeadReckoningWheel::delta_angle_to_rotation_matrix(Pose &delta_dr_transform)
{
